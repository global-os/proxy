...
export const replaceDomainInHTML = (
    ...
    try {
        ...
        doc.querySelectorAll('meta[http-equiv="refresh"]').forEach((el: HTMLMetaElement) => {
            const content = el.getAttribute('content');
            if (content) {
                el.setAttribute('content', content.replace(new RegExp(oldDomain, 'gi'), newDomain));
            }
        });
        ...
        doc.querySelectorAll('[style]').forEach((el: HTMLStyleElement) => {
            ...
            if (style && style.includes(oldDomain)) {
                el.setAttribute('style', style.replace(new RegExp(oldDomain, 'gi'), newDomain));
            }
        });
        ...
        doc.querySelectorAll('style').forEach((el: HTMLStyleElement) => {
            if (el.textContent.includes(oldDomain)) {
                el.textContent = el.textContent.replace(new RegExp(oldDomain, 'gi'), newDomain);
            }
        });

        // Replace in inline <script> tags
        doc.querySelectorAll('script:not([src])').forEach((el: HTMLScriptElement) => {
            if (el.textContent.includes(oldDomain)) {
                el.textContent = el.textContent.replace(new RegExp(oldDomain, 'gi'), newDomain);
            }
        });
