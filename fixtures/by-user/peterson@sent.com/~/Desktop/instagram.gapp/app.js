...
function showError(msg) {
  console.error('[instagram.gapp] error:', msg)
...
window.addEventListener('message', function(event) {
  ...
  if (data.type === 'init:fresh' || data.type === 'init') {
    ...
    window.parent.postMessage({ type: 'webview:create', requestId: pendingRequestId, domain: 'instagram.com' }, '*')
    return
  }
...
console.log('[instagram.gapp] loaded, sending ready')
